...
def calulateHarmonic(imageCollection:ee.ImageCollection, dependent:ee.String):
    ...
    phase = harmonicTrendCoefficients.select('cos').atan2(
                harmonicTrendCoefficients.select('sin')).rename(dependent.cat('_phase'))
                
    amplitude = harmonicTrendCoefficients.select('cos').hypot(
                    harmonicTrendCoefficients.select('sin')).rename(dependent.cat('_amplitude'))
...
if __name__ == '__main__':
    ...
    filteredCollection = imageCollection.filterBounds(roi).filterDate('2019-01-01','2021-01-01')
    
    # test add time bands
    timeCollection = addTimeConstant(filteredCollection,timeField)

    assert timeCollection.select('t','constant').first().bandNames().getInfo() == ['t','constant']
    
    harmonics = calulateHarmonic(timeCollection, dependent)
    # test harmonics returns correect bands
    assert harmonics.bandNames().getInfo() == ee.List([dependent.cat('_phase'), dependent.cat('_amplitude')]).getInfo()
