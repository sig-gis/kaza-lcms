...
if __name__ == "__main__":
	...
	END_DATE = ee.Date.fromYMD(year,month+3,30)
	...
	outputName = "projects/sig-ee/WWF_KAZA_LC/" + str(year)  +str(month).zfill(2) + "_" + str(month+4).zfill(2)

	task_ordered = ee.batch.Export.image.toAsset(image=ee.Image(img).clip(aoi).toInt(), description="Export "+str(year)+"sentinel2_aoi", assetId=outputName,region=region.getInfo()['coordinates'], maxPixels=1e13,scale=10 )
